...
describe('cacheController', function(){
    ...
    describe('getUrl', function(){
        ...
        beforeEach(function(){
            ...
            skipCacheStub = sandbox.stub(bypass, 'skipCache');
        });

        it('checks to see if the URL should bypass the cache by calling bypass.skipCache', function(){
            cacheController.getUrl(testUrl, callbackStub);
            expect(skipCacheStub.calledOnce).toEqual(true);
        });

        describe('if the url should not be bypassed', function(){
            beforeEach(function(){
                skipCacheStub.returns(false);
                cacheController.getUrl(testUrl, callbackStub);
            });
        ...
        describe('if the url should bypass the cache', function(){
            var getDataStub;

            beforeEach(function(){
                getDataStub = sandbox.stub(fetcher, 'getData');
                skipCacheStub.returns(true);
                cacheController.getUrl(testUrl, callbackStub);
            });
